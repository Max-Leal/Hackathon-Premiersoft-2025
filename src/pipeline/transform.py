...
# --- FUNÇÕES DE AUTOSSUFICIÊNCIA (SEM ALTERAÇÃO) ---
...
def fetch_dependency_data(engine, table_name: str) -> pd.DataFrame:
    logging.info(f"Dependência '{table_name}' não fornecida. Buscando do banco de dados...")
    try:
        df = pd.read_sql_table(table_name, engine)
        logging.info(f"Encontrados {len(df)} registros para '{table_name}' no banco.")
        return df
    except Exception:
        logging.warning(f"Tabela de dependência '{table_name}' não encontrada ou vazia no banco. Um DataFrame vazio será usado.")
        return pd.DataFrame()

# --- SUAS FUNÇÕES AUXILIARES (SEM ALTERAÇÃO) ---
...
def run(dataframes: Dict[str, pd.DataFrame | Iterator]) -> Dict[str, pd.DataFrame | Iterator]:
    logging.info("Iniciando a etapa de transformação autossuficiente...")

    DEPENDENCY_ENTITIES = ['municipios', 'estados', 'cid10', 'hospitais']
    engine = None
    for entity in DEPENDENCY_ENTITIES:
        if entity not in dataframes:
            if not engine: engine = get_database_engine()
            dataframes[entity] = fetch_dependency_data(engine, entity)
    if engine: engine.dispose()
        
    logging.info("Todas as dependências foram satisfeitas. Prosseguindo com as transformações.")
    ...
    if df_municipios is not None and not df_municipios.empty:
        ...
        # --- NOVO BLOCO DE HARMONIZAÇÃO DE SCHEMA ---
        # Se os dados vieram do banco, a coluna 'localizacao' existe, mas 'latitude'/'longitude' não.
        if 'localizacao' in df_municipios.columns and 'latitude' not in df_municipios.columns:
            logging.info("Dados de municípios vieram do banco. Extraindo lat/lon da coluna de geometria.")
            # Acessa as coordenadas do objeto de geometria. O .y é a latitude, o .x é a longitude.
            # Adiciona a verificação 'if geom' para tratar geometrias nulas.
            df_municipios['latitude'] = df_municipios['localizacao'].apply(lambda geom: geom.y if geom else None)
            df_municipios['longitude'] = df_municipios['localizacao'].apply(lambda geom: geom.x if geom else None)
        # --- FIM DO NOVO BLOCO ---
    ...
    valid_cid_codes = set()
    if df_cid10 is not None and not df_cid10.empty and 'codigo' in df_cid10.columns:
        valid_cid_codes = set(df_cid10['codigo'].dropna().astype(str))
        logging.info(f"Encontrados {len(valid_cid_codes)} códigos CID-10 válidos para validação.")
    ...
    def process_single_pacientes_chunk(chunk_data):
        ...
        try:
            ...
            if processed_chunk.empty: return pd.DataFrame()
            ...
            processed_chunk.loc[~processed_chunk['cod_municipio'].isin(valid_municipio_ids), 'cod_municipio'] = pd.NA
            if 'cid_10' in processed_chunk.columns:
                processed_chunk['cid_10'] = processed_chunk['cid_10'].astype(str)
                processed_chunk.loc[~processed_chunk['cid_10'].isin(valid_cid_codes), 'cid_10'] = None
    ...
    if df_cid10 is not None and not df_cid10.empty: df_cid10['especialidade'] = df_cid10['codigo'].astype(str).apply(get_especialidade_from_cid)
